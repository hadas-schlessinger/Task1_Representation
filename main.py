...
def main():
    ...
    split_data = dp.train_split_data(dand_l['Data'], dand_l['Labels'], params['Split']) # returns train data, test data, train labels and test labels
    train_data_rep = dp.prepare(split_data['Train']['Data'], params['Prepare'])
    train_model = model.train(train_data_rep, split_data['Train']['Labels'], params['Train'])
